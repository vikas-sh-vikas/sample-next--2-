...
function Page() {
  ...
  const [data, setData] = useState<UnitListModel[]>([]); // State to store the data
    ...
    if (isLoading) {
      ...
      return data.map((item, index) => (
        <TableRow key={item.id}>
          ...
          <TableColumn variant="leftAlign">{item.unitName}</TableColumn>
  ...
  return (
    <div className="border-4 ">
      <PageHeader icon={<FaRegFileAlt />} title={"Units"} size="large">
        <Search
          searchText={searchText}
