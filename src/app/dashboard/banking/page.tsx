...
import { GetItemsList, GetUserBankList } from "@/utils/api.constant";
...
function Page() {
  ...
  const [data, setData] = useState<TUserBankListodel[]>([]); // State to store the data
    ...
    if (isLoading) {
      ...
      return data.map((item, index) => (
        <TableRow key={item.id}>
          ...
          <TableColumn variant="leftAlign">{item.bankName}</TableColumn>
          <TableColumn variant="leftAlign">{item.ifscCode}</TableColumn>
          <TableColumn variant="leftAlign">{item.accountNumber}</TableColumn>
          <TableColumn variant="leftAlign">{item.branchName}</TableColumn>
  ...
  return (
    <div className="border-4 ">
      <PageHeader icon={<FaRegFileAlt />} title={"Banks"} size="large">
        <Search
          searchText={searchText}
