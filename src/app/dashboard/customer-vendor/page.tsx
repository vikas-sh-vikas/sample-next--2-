...
import { GetCompanyList } from "@/utils/api.constant";
...
function Page() {
    ...
    if (isLoading) {
      ...
      return data?.map((item, index) => (
        <TableRow key={item.id}>
          ...
          <TableColumn variant="leftAlign">{item.companyName}</TableColumn>
          <TableColumn variant="leftAlign">{item.gstinNo}</TableColumn>
          <TableColumn variant="leftAlign">
  ...
  return (
    <div className="border-4 ">
      <PageHeader icon={<FaRegFileAlt />} title={"Party Ledger"} size="large">
        <Search
          searchText={searchText}
